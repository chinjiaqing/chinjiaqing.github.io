...
function renderArticleAsideNav(template = "") {
	...
	template = template.replace(pattern, function (all, num, text) {
		...
		html += util.format('<li class="level%s"><a href="#%s">%s</a></li>', num, index.join("."), text);
...
function parseMarkdownHead(content = ""){
  ...
  if (noteMatch) {
    ...
    result.create_at =
      createDateMatch && createDateMatch[1] ? createDateMatch[1].trim() : "";
    result.update_at =
      updateDateMatch && updateDateMatch[1] ? updateDateMatch[1].trim() : "";
